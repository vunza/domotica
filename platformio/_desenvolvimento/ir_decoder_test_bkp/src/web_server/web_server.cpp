...
//ConfigDados espConfigData; // (*J*) Nao crie objectos da estructura na funcao loo() 
...
void WebServer::begin() {
    ...
    //espConfigData = lerDadosEEPROM();
    //lerDadosEEPROM(&espConfigData);
    ...
    server->on("/api/rename_esp", HTTP_POST, [](AsyncWebServerRequest *request){},NULL,[](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
        ...
        if (nome.length() > 0) {
            ...
            //strncpy(espConfigData.device_name, nome.c_str(), DEVICE_NAME_SIZE); 
            //salvarDadosEEPROM(espConfigData);
            //salvarDadosEEPROM(&espConfigData);
    ...
    server->on("/api/get_esp_name", HTTP_GET, [](AsyncWebServerRequest *request){  
        ...
        //request->send(200, "text/plain",  String(espConfigData.device_name) + "," + String(DEVICE_NAME_SIZE)); 
    });
