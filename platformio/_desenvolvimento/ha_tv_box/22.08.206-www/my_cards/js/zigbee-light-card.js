...
import { lightCardStyles } from '../css/zigbee-light-card-styles.js';
...
import { lightCardTemplate } from '../html/zigbee-light-card-template.js';
...
class ZigbeeLightCard extends HTMLElement {
  static getStubConfig() {
    return { entity: 'light.lampada_inteligente', name: '', show_time: true };
  }
  ...
  setConfig(config) {
    if (!config.entity || (!config.entity.startsWith('light.') && !config.entity.startsWith('switch.') && !config.entity.startsWith('plug.'))) {
      throw new Error('Defina uma entidade válida (ex: light.luz_sala ou switch.lampada_quarto)');
    }
    this._config = { ...config };
  ...
  // Mapeia e atualiza a entidade automaticamente caso seja renomeada no Home Assistant
    ...
    try {
      ...
      if (oldEntry && oldEntry.device_id) {
        const newEntry = registry.find(entry => 
          ...
          (entry.entity_id.startsWith('light.') || entry.entity_id.startsWith('switch.') || entry.entity_id.startsWith('plug.')) &&
          !entry.disabled_by
        );
        ...
        if (newEntry && newEntry.entity_id !== this._config.entity) {
          ...
          ['power_sensor', 'voltage_sensor', 'current_sensor', 'energy_sensor'].forEach(key => {
            if (this._config[key]) {
    ...
    if (isUnavailable) {
      ...
      if (this._deviceMac) {
        newEntityId = Object.keys(this._hass.states).find(id => {
          if (!id.startsWith('light.') && !id.startsWith('switch.') && !id.startsWith('plug.')) return false;
      ...
      if (newEntityId && newEntityId !== this._config.entity) {
        ...
        ['power_sensor', 'voltage_sensor', 'current_sensor', 'energy_sensor'].forEach(sensorKey => {
  ...
  async _render() {
    ...
    const initialTitle = this._config.name || 'Luz Zigbee';
    ...
    shadow.innerHTML = `
      ...
      ${lightCardTemplate(initialTitle, clockSvg, menuTemplate)}
      ${renameModalTemplate}      
      ${timerModalTemplate(this._config.name || this._config.entity.split('.')[1] || 'Luz Zigbee')}
    `;
    ...
    try {
      ...
      console.error('Erro ao inicializar dados dos timers do card light:', err);
    }
  ...
  _toggleEntity(e) {
    ...
    const domain = this._config.entity.split('.')[0] || 'light';
  ...
  _updateContent() {
    ...
    if (!this._stateObj) {
      ...
      if (cardIcon) {
        cardIcon.setAttribute('icon', 'mdi:lightbulb-off');
      }
      return;
    }
    ...
    if (iconSpan) {
      iconSpan.style.color = isOn
        ? 'var(--paper-item-icon-active-color, #ffca28)'
        : 'var(--paper-item-icon-color, #8e8e93)';
    }

    if (cardIcon) {
      cardIcon.setAttribute('icon', isOn ? 'mdi:lightbulb' : 'mdi:lightbulb-off');
    }
  ...
  async _fetchHistoryIfNeeded() {
    ...
    try {
      ...
      console.error('Erro ao obter histórico da luz:', err);
      this._lastHistoryFetch = 0;
    }
  }
  ...
  _renderGraph(historyData, sensors) {
    ...
    if (!hasTelemetry) {
      ...
      pathsHtml += `<path d="${d}" fill="none" stroke="#ffca28" stroke-width="2.5" />`;
...
if (!customElements.get('zigbee-light-card')) {
  customElements.define('zigbee-light-card', ZigbeeLightCard);
}