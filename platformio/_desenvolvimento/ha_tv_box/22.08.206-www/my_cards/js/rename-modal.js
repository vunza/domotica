 ...
 * @param {Object} cardInstance - Instância do ZigbeePlugCard (this).
...
export function initRenameModal(cardInstance, shadow) {
  ...
  saveBtn.addEventListener('click', () => {
    ...
    const entityId = cardInstance._config.entity || saveDeviceId.value;
    ...
    .then(() => {
      // 1. Atualiza os dados de nome na instância do card
      cardInstance._customNameProvided = true;
      cardInstance._customName = newName;
      cardInstance._config = { ...cardInstance._config, name: newName };

      // 2. Notifica o Lovelace para persistir as alterações na Dashboard
      cardInstance.dispatchEvent(new CustomEvent('config-changed', {
        detail: { config: cardInstance._config },
        bubbles: true,
        composed: true
      }));

      // 3. Força a renderização imediata do cabeçalho do card
      if (typeof cardInstance._updateContent === 'function') {
        cardInstance._updateContent();
      } else {
    ...
    .catch((error) => {
      console.error('Erro ao chamar serviço de renomear:', error);
  ...
  /**
   * Abre o modal de renomear preenchendo com a entidade e nome atuais.
   */
